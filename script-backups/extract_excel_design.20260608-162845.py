...
def group_cells_for_sections(
    ...
    def is_inline_row_content_title(title: dict) -> bool:
        row_cells = [
            cell
            for cell in cells_by_row_without_candidate_titles.get(title["row"], [])
            if cell.get("text", "").strip()
        ]
        preceding_cells = [
            cell
            for cell in row_cells
            if cell_center_col(cell) < cell_center_col(title)
        ]
        if not preceding_cells:
            return False
        title_range = title_range_key(title)
        preceding_same_range = [
            cell
            for cell in preceding_cells
            if title_range[0] <= cell_center_col(cell) <= title_range[1]
        ]
        if not preceding_same_range:
            return False
        for row_number, row_cells in cells_by_row_without_candidate_titles.items():
            if row_number >= title["row"]:
                continue
            if any(
                title_range[0] <= cell_center_col(cell) <= title_range[1]
    ...
    for index, title in enumerate(sorted_title_candidates):
        ...
        if is_inline_row_content_title(title):
            demoted_title_refs.add(title["ref"])
