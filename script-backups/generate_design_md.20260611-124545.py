...
def visual_stack_table_section(
    ...
    for row_group in image_groups(image_group):
        ...
        if image_local_rows:
            output.append("")
            output.extend(render_rows(image_local_rows))
        output.append("")
    group_right = max(image.to_col for image in image_group)
...
def parse_one(path: Path, output_dir: Path) -> dict:
    ...
    for title in titles:
        ...
        mixed_body = render_mixed_list_table_section(section["cells"]) if not section["images"] else None
        if mixed_body is not None:
            body = mixed_body
        elif use_card_rendering(section["cells"], section["images"]):
            body = render_card_section(section["cells"], section["images"], styles)
        else:
            body = split_rows_for_images(lines_by_row, section["images"], (title.start_col, title.end_col))
        if body:
            lines.extend(body)
            lines.append("")
