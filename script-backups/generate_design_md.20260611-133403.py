...
def split_list_table_cells(cells: list[Cell]) -> tuple[list[Cell], list[Cell]] | None:
    ...
    if not left_cells or not right_cells:
        return None
    right_cols = {cell.start_col for cell in right_cells}
    right_multi_rows = {
        cell.row
        for cell in right_cells
        if sum(1 for item in right_cells if item.row == cell.row) >= 3
    }
    if len(right_cols) < 3 or not right_multi_rows:
        return None
...
def visual_stack_table_section(
    ...
    adjusted_rows, local_rows_by_image = split_image_local_list_rows(adjusted_rows, image_group)
...
def split_image_local_list_rows(
    ...
    row_numbers = {row for row, _items in rows}
    for image in images[1:]:
        ...
        image_cluster = [
            item
            for cluster in row_clusters(image_cluster)
            if cluster[0][0] <= image.from_row and cluster[-1][0] >= image.from_row
            for item in cluster
        ]
        image_cluster_cols = {start for _row, items in image_cluster for start, _text in items}
        if image_cluster and len(image_cluster_cols) >= 2 and table_shape_detected(image_cluster):
...
def parse_one(path: Path, output_dir: Path) -> dict:
    ...
    for cell in cells:
        ...
        if title is not None:
            ...
            if right_title is not None and same_row_visual_title is None:
                title = right_title
    ...
    for title in titles:
        ...
        mixed_body = render_mixed_list_table_section(section["cells"]) if not flow_images else None
        if mixed_body is not None:
            body = mixed_body
        elif use_card_rendering(section["cells"], flow_images):
            body = render_card_section(section["cells"], flow_images, styles)
        else:
