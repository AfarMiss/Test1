...
def group_cells_for_sections(cells: list[dict], title_styles: set[str | None], images: list[dict]) -> list[dict]:
    ...
    for image in images:
        ...
        if row is None or col is None:
            ...
            append_to_section(section_for_row(row), image_block(image))
...
def public_section_blocks(blocks: list[dict]) -> list[dict]:
    row_like_blocks = [
        block
        for block in blocks
        if block.get("type") in ("row", "text") and any(line for line in block.get("lines", []))
    ]
    table_like_blocks = [block for block in blocks if block.get("type") == "table"]
    image_like_blocks = [block for block in blocks if block.get("type") == "image"]
...
def public_sections(blocks: list[dict]) -> list[dict]:
    ...
    for section in sections:
        if section.get("title") is not None and not section.get("blocks") and compacted:
            compacted[-1].setdefault("blocks", []).append({"lines": [section["title"]]})
