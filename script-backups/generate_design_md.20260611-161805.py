...
def primary_left_visual_images(
    ...
    candidates = [
        group
        for group in image_column_groups(images)
        if max(image.to_col for image in group) < text_min_col
    ]
    if not candidates:
        return []
    return max(
...
def primary_visual_table_section(
    ...
    if len(images) < 2 or not table_shape_detected(rows):
        return None
    ...
    for image in sorted(auxiliary_images, key=lambda item: (item.from_row, item.from_col)):
        ...
        suffixes[lane_index].setdefault(target_row, []).append(image_line)

    compacted_lanes: list[list[str]] = []
    for lane_index, items in enumerate(lane_items):
        compacted = []
        for row_number, value in items:
            suffix = " ".join(suffixes[lane_index].get(row_number, []))
...
def parse_one(path: Path, output_dir: Path) -> dict:
    ...
    for title in ordered_titles:
        ...
        primary_table_body = (
            primary_visual_table_section(lines_by_row, flow_images)
            if flow_images
            else None
        )
