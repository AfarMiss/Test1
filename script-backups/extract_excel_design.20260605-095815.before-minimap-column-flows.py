...
def line_for_cells(cells: list[dict]) -> str:
    return " ".join(cell["text"].strip() for cell in cells if cell.get("text", "").strip())


def has_distinct_parallel_flow_text(
    rows: dict[int, list[dict]],
    range_keys: list[tuple[int, int]],
) -> bool:
    for row_cells in rows.values():
        row_lines = []
        for start_col, end_col in range_keys:
            line = line_for_cells(
                [
                    cell
                    for cell in row_cells
        ...
        if len(row_lines) > 1 and len(set(row_lines)) > 1:
            return True
    return False
...
def group_cells_for_sections(cells: list[dict], title_styles: set[str | None], images: list[dict]) -> list[dict]:
    ...
    for section_index, rows in grouped_by_section_row.items():
        ...
        use_column_flows = len(range_keys) > 1 and has_distinct_parallel_flow_text(rows, range_keys)
        if use_column_flows:
            for flow_order, range_key in enumerate(range_keys):
                ...
                for row_number in sorted(rows):
                    ...
                    block = block_from_cells(row_number, row_cells)
                    if block is not None:
    ...
    title_by_section_id = {
        id(section_by_title[title["ref"]]): title
        for title in sorted_titles
    }
    rows_by_section_id: dict[int, set[int]] = {}
    for section in sections:
        rows_by_section_id[id(section)] = {
            block_bounds(block)["start_row"]
            for block in section["blocks"]
            if block.get("type") == "row"
        }
    moves: list[tuple[dict, dict, dict]] = []
    image_rows_by_section_id: dict[int, set[int]] = {}
    for section in sections:
        rows: set[int] = set()
        for block in section["blocks"]:
            if block.get("type") != "image":
                continue
            bounds = block_bounds(block)
            rows.update(range(bounds["start_row"], bounds["end_row"] + 1))
        image_rows_by_section_id[id(section)] = rows
    for source_section in sections:
        source_title = title_by_section_id.get(id(source_section))
        if source_title is None:
            continue
        source_body_start = min(
            (
                block_bounds(block)["start_row"]
        ...
        row_blocks = [block for block in source_section["blocks"] if block.get("type") == "row"]
        source_col_ranges_at_row: dict[int, list[tuple[int, int]]] = {}
        for block in row_blocks:
            bounds = block_bounds(block)
            source_col_ranges_at_row.setdefault(bounds["start_row"], []).append(
                (bounds["start_col"], bounds["end_col"])
            )
        flow_cols: set[int] = set()
        flow_targets: dict[int, dict] = {}
        for block in row_blocks:
            bounds = block_bounds(block)
            candidates = [
                title
                for title in sorted_titles
                if len(title_rows[title["row"]]) == 1
                and title["row"] > source_title["row"]
                and source_body_start is not None
                and title["row"] > source_body_start
                and title["row"] <= bounds["start_row"]
                and bounds["start_row"] in rows_by_section_id[id(section_by_title[title["ref"]])]
                and bounds["start_row"] in image_rows_by_section_id[id(section_by_title[title["ref"]])]
                and not source_col_ranges_at_row.get(title["row"])
            ]
            if not candidates:
                continue
            target_title = max(candidates, key=lambda cell: cell["row"])
            flow_cols.add(bounds["start_col"])
            flow_targets[bounds["start_col"]] = section_by_title[target_title["ref"]]
        for block in row_blocks:
            bounds = block_bounds(block)
            if bounds["start_col"] in flow_cols:
                target_section = flow_targets[bounds["start_col"]]
                target_title = title_by_section_id[id(target_section)]
                if bounds["start_row"] >= target_title["row"]:
                    moves.append((source_section, target_section, block))
    for source_section, target_section, block in moves:
        source_section["blocks"].remove(block)
        block["_flow_order"] = 1
    ...
    for block in sorted(
        ...
        if public is not None:
            if public.get("type") == "image":
                image_indexes.extend(public.get("image_indexes", []))
            elif public.get("type") == "text":
                text_lines.extend(public.get("lines", []))
            else:
                table_blocks.append(public)

    public_blocks: list[dict] = []
    append_public_text(public_blocks, text_lines)
    public_blocks.extend(table_blocks)
    append_public_image(public_blocks, image_indexes)
...
def public_sections(blocks: list[dict]) -> list[dict]:
    ...
    for block in blocks:
        ...
        if public is None:
            continue
        if public.get("type") == "section":
            sections.append(public)
        else:
            loose_blocks.append(public)
    if loose_blocks and not sections:
        sections.insert(0, {"type": "section", "title": None, "blocks": loose_blocks})
    while len(sections) > 1 and sections[0].get("title") is None:
        sections.pop(0)
    compacted = []
    for section in sections:
        if section.get("title") is not None and not section.get("blocks") and compacted:
            compacted[-1].setdefault("blocks", []).append({"type": "text", "lines": [section["title"]]})
            compacted[-1]["blocks"] = public_section_blocks(compacted[-1]["blocks"])
            continue
