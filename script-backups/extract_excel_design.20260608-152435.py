...
def group_cells_for_sections(
    ...
    initial_anchor_title_refs: set[str] = set()
    anchor_title_rows: set[int] = set()
    subsequent_anchor_title_rows: set[int] = set()
    if not title_candidates:
        layout_title_candidates = first_row_layout_title_cells()
        layout_title_refs = {cell["ref"] for cell in layout_title_candidates}
        anchor_title_candidates = image_anchor_title_cells()
        anchor_title_rows = {cell["row"] for cell in anchor_title_candidates}
        if anchor_title_candidates:
            first_anchor_row = min(cell["row"] for cell in anchor_title_candidates)
            initial_anchor_title_refs = {
                cell["ref"] for cell in anchor_title_candidates if cell["row"] == first_anchor_row
            }
            subsequent_anchor_title_rows = {
                cell["row"] for cell in anchor_title_candidates if cell["row"] != first_anchor_row
            }
        title_candidates = layout_title_candidates + anchor_title_candidates
    ...
    def row_band_title_after(
        ...
        for title in sorted_titles:
            ...
            if title["row"] <= baseline_title["row"]:
                continue
            if baseline_title["ref"] in layout_title_refs and title["ref"] in initial_anchor_title_refs:
                continue
    ...
    for section_index, rows in grouped_by_section_row.items():
        ...
        section_title_ref = section["source_refs"][0] if section.get("source_refs") else None
        row_segments: list[dict[int, list[dict]]] = []
        current_segment: dict[int, list[dict]] = {}
        for row_number in sorted(rows):
            layout_number_sync_split = False
            if section_title_ref in layout_title_refs:
                    section_title = title_by_ref.get(section_title_ref or "")
                    if section_title is not None:
                        section_range = column_range_for_col(section_title["col"], ranges)
                        section_range_key = (section_range["start_col"], section_range["end_col"])
                    numbered_rows = [
                        numbered_row
                        for numbered_row in row_number_by_range_key.get(section_range_key, {})
                        if numbered_row <= row_number
                    ]
                    active_layout_numbers = (
                        row_number_by_range_key[section_range_key][max(numbered_rows)]
                        if numbered_rows
                        else set()
                    )
                    explicit_layout_numbers = row_number_by_range_key.get(section_range_key, {}).get(row_number, set())
                    other_title_numbers: set[str] = set()
                    for title_ref, numbers_by_row in row_number_by_title_ref.items():
                        if title_ref == section_title_ref:
                            continue
                        other_title_numbers.update(numbers_by_row.get(row_number, set()))
                    layout_number_sync_split = bool(
                        active_layout_numbers
                        and other_title_numbers
                        and active_layout_numbers.intersection(other_title_numbers)
                        and not explicit_layout_numbers
                    )
            if (
                (
                    row_number in demoted_titles_by_row
                    or (
                        section_title_ref in layout_title_refs
                        and row_number in subsequent_anchor_title_rows
                    )
                    or layout_number_sync_split
                )
                and current_segment
            ):
