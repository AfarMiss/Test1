...
def add_top_preamble_titles(cells: list[Cell], titles: list[Cell], styles: dict[str, dict], lanes: list[int]) -> list[Cell]:
    added: list[Cell] = []
    for cell in cells:
        if cell.row != 1 or cell.start_col <= 1 or not cell.text.strip():
            continue
        feature = styles.get(cell.style or "", {})
        if feature.get("filled") or not feature.get("bold"):
            continue
        same_lane_titles = [
            title
            for title in titles
            if title.row > cell.row
            and horizontal_overlap(cell.start_col, cell.end_col, *lane_bounds_for_title(title, lanes)) > 0
        ]
        if not same_lane_titles:
            continue
        first_title_row = min(title.row for title in same_lane_titles)
...
def split_image_local_list_rows(
    ...
    local_rows: set[int] = set()
    image_headings = image_headings or {}
    for image in images[1:]:
        if not image_headings.get(image.index):
            image_cluster = [
                (row, items)
                for row, items in rows
            ...
            if (
                ...
                local_rows.update(row for row, _items in image_cluster)
                continue
        candidates = [
            (row, items)
            for row, items in rows
        ...
        if len(candidates) >= 2 and not table_shape_detected(candidates):
            ...
            local_rows.update(row for row, _items in candidates)
    return [item for item in rows if item[0] not in local_rows], by_image
...
def grouped_row_texts(cells: list[Cell]) -> list[tuple[int | None, list[str]]]:
    ...
    for _row, items in lines:
        text = " ".join(item_text for _start, item_text in items).strip()
        row_number = first_number(text)
        if row_number is not None or not groups:
...
def render_paired_rule_table(section_cells: list[Cell], rule_cells: list[Cell]) -> list[str] | None:
    left_groups = grouped_row_texts(section_cells)
    right_groups = grouped_row_texts(rule_cells)
    if not left_groups or not right_groups:
        return None

    right_by_number: dict[int | None, list[list[str]]] = {}
    for number, rows in right_groups:
        right_by_number.setdefault(number, []).append(rows)

    table_rows: list[tuple[str, str]] = []
    consumed_right_ids: set[int] = set()
    for number, left_rows in left_groups:
        right_rows: list[str] = []
        candidates = right_by_number.get(number, [])
        if candidates:
            right_rows = candidates.pop(0)
            consumed_right_ids.add(id(right_rows))
    ...
    for _number, rows_list in right_by_number.items():
        for right_rows in rows_list:
            if id(right_rows) in consumed_right_ids:
                continue
...
def parse_one(path: Path, output_dir: Path) -> dict:
    with zipfile.ZipFile(path) as zf:
        ...
        lanes = title_lanes(titles)
        titles = add_top_preamble_titles(cells, titles, styles, lanes)
        lanes = title_lanes(titles)
    ...
    for title in ordered_titles:
        ...
        aligned_rule_cells = align_paired_rule_cells_to_section(section["cells"], rule_cells_for_title)
        render_cells = [*section["cells"], *aligned_rule_cells]
        ...
        paired_body = (
            render_paired_rule_section(flow_images, section["cells"], rule_cells_for_title)
        ...
        if paired_body is not None:
            ...
            body = split_rows_for_images(lines_by_row, flow_images, (title.start_col, title.end_col))
        if body:
