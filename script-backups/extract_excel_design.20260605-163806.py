...
def group_cells_for_sections(
    ...
    for index, title in enumerate(sorted_title_candidates):
        ...
        if fallback_title_styles and (no_own_text_continuation or numbered_continuation):
    ...
    def has_synchronized_numbered_flow(title: dict, baseline_range_key: tuple[int, int], row: int) -> bool:
    ...
    def row_band_title_after(
        ...
        if protect_baseline_image_band and any(
            start_row <= row <= end_row
            for start_row, end_row in image_rows_by_title_ref.get(baseline_title["ref"], [])
        ):
            return None
        row_band_candidates = []
        for title in sorted_titles:
            ...
            if not any(
                start_row <= row <= end_row
                for start_row, end_row in image_rows_by_title_ref.get(title["ref"], [])
            ):
                continue
            if has_synchronized_numbered_flow(title, baseline_range_key, row):
    ...
    def section_for_position(row: int, col: int, protect_baseline_image_band: bool = True) -> dict:
        col_range = column_range_for_col(col, ranges)
        range_key = (col_range["start_col"], col_range["end_col"])
        candidates = [
            title
        ...
        if not candidates:
            return sections[0]
        same_range_title = max(candidates, key=lambda cell: cell["row"])
        row_band_title = row_band_title_after(row, same_range_title, range_key, protect_baseline_image_band)
        if row_band_title:
            return section_by_title[row_band_title["ref"]]
        return section_by_title[same_range_title["ref"]]
    ...
    for section_index, rows in grouped_by_section_row.items():
        ...
        for segment_rows in row_segments:
            ...
            flow_ranges = text_flow_ranges_from_rows(segment_rows) if fallback_title_styles else []
            range_keys = (
    ...
    for image in images:
        ...
        if row is None or col is None:
            ...
            append_to_section(section_for_position(row, col, False), image_block(image))
